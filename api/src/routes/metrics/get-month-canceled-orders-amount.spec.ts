...
describe("List the monthly amount of canceled orders [GET] /metrics/month-canceled-orders-amount", () => {
  beforeAll(async () => {
    await app.ready()
    ...
    for (let index = 0; index < 30; index++) {
      ...
      await prisma.order.update({
        ...
        data: {
          status: faker.helpers.arrayElement(["CANCELED", "DELIVERED"]),
  ...
  it("should be able to list the monthly amount of canceled orders", async () => {
    const response = await request(app.server)
