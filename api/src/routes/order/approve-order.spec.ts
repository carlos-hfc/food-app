...
describe("Approve order [PATCH] /orders/:orderId/approve", () => {
  beforeAll(async () => {
    await app.ready()
  })
  ...
  it("should not be able to approve an order with closed restaurant", async () => {
    const { orderId, restaurantToken } = await createOrder(app)
  ...
  it("should not be able to approve an inexistent order", async () => {
    const { restaurantToken } = await createOrder(app)

    vi.setSystemTime(new Date(2025, 7, 21, 13, 0, 0))

    const response = await request(app.server)
      .patch(`/orders/${randomUUID()}/approve`)
      .set("Cookie", restaurantToken)
      .send()
  ...
  it("should not be able to approve an order without the pending status", async () => {
    const { orderId, restaurantToken } = await createOrder(app, {
      status: "PREPARING",
    })
